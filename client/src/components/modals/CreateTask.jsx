...
import {createTask, fetchStatuses, fetchProjects} from "../../http/tasksAPI";
...
const CreateTask = observer(({show, onHide}) => {
	...
	const addTask = async () => {
		let data;
			data = await createTask(title, startDate, `${planned}`, task.selectedProject.id, task.selectedStatus.id, description)
		onHide()
		setDescription('')
		setStartDate('')
		setPlanned(1)
											...
											<div className="mt-2">
												<form>
													<div className="mb-6">
														...
														<Listbox value={task.selectedProject.title} onChange={task.setSelectedProject.title}>
															<div className="relative mt-1">
																<Listbox.Button
																	className="text-left bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
																		<span
																			className="block truncate">{task.selectedProject.title || "Выберите проект"}</span>
																	<span
																...
																<Listbox.Options
																	className="absolute mt-1 max-h-[10rem] w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
																	{task.projects.map(project => (
																		<Listbox.Option
																			onClick={() => task.setSelectedProject(project)}
																			key={project.id}
																			className={({selected}) =>
													...
													<div className="mb-6">
														...
														<Listbox value={task.selectedStatus.title} onChange={task.setSelectedStatus.title}>
															<div className="relative mt-1">
																<Listbox.Button
																	className="text-left bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
																		<span
																			className="block truncate">{task.selectedStatus.title || "Выберите статус"}</span>
																	<span
																		className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
																			<ChevronUpDownIcon
																				className="h-5 w-5 text-gray-400"
																...
																<Listbox.Options
																	className="absolute mt-1 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
																	{task.statuses.map(status => (
																		<Listbox.Option
																			onClick={() => task.setSelectedStatus(status)}
																			key={status.id}
																			className={({selected}) =>
																				`relative cursor-pointer select-none py-2 pl-10 pr-4 ${
																					selected ? 'bg-gray-100 text-gray-900' : 'text-gray-900 hover:bg-gray-100'
